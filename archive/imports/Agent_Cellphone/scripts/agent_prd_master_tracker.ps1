param(
	[string]$Root = 'D:\repos',
	[string]$MasterTracker = 'D:\repos\runtime\agent_comms\prd_master_tracker.json',
...
foreach ($repo in $repos) {
	try {
		...
		if (-not $entry) {
			...
			$tracker += $entry
			Write-Log "Added PRD tracker for $repoName (Agent: $AgentName)"
		}
		else {
...
if (-not $DryRun) { Save-Tracker $MasterTracker $tracker }

Write-Log "Global PRD tracker updated at $MasterTracker"
